...
$sql = "SELECT * FROM users where username='".$username."' and password='".$password."'";//根据username获取用户
...
if(!empty($user_info)) {
	// echo 1;//1 如果这个查找出来的数组有长度，说明有此用户，执行登录1状态
	$ret = array(
    'status' => 1,
	);
	...
	// echo 0;//0 不存在 如果查找出来的数组为空，则说明没有此用户，执行登录失败0
	$ret = array(
    'status' => 0,
	);
	echo json_encode($ret);
